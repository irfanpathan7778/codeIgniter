...
class ExpenseController extends MY_Controller {
    ...
    public function add_expense() {
        ...
        $data = [
            'user_id' => $user_id,
            'amount' => $input_data['amount'] ?? 0,
            'category_id' => $input_data['category_id'] ?? 0,
        ...
        $budget_limit = $this->BudgetModel->get_budget_by_category($user_id, $data['category_id']);
        
        $total_spent = $this->ExpenseModel->get_total_spent_by_category($user_id, $data['category_id']);
    
        if ($budget_limit && ($total_spent + $data['amount']) > $budget_limit) {

            $notification_data = [
                'category_id' => $data['category_id'],
                'description' => 'Your budget is exceeded for this category!'
            ];
            $this->NotificationModel->store_notification($notification_data);
            echo json_encode(["message" => "Your budget is exceeded for this category!"]);
            return;


        }
        // print_r($total_spent);exit;
    ...
    public function get_notifications(){
        ...
        echo json_encode(['notification'=> $notifications]);
    ...
    public function mark_read(){
        ...
        log_message('debug', 'mark_read API called');
    
        $this->load->model('NotificationModel');
        $this->NotificationModel->mark_notifications_read();
